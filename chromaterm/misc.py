...
- description: IPv6
  regex: (?i)\b(?<!:)(::(ffff(:0{1,4})?:)?((25[0-5]|(2[0-4]|[0-1]?\d)?\d)\.){3}(25[0-5]|(2[0-4]|[0-1]?\d)?\d)|((?P<h>[\da-f]{1,4}):){1,4}:((25[0-5]|(2[0-4]|[0-1]?\d)?\d)\.){3}(25[0-5]|(2[0-4]|[0-1]?\d)?\d)|(([0-9a-f]):){7}([0-9a-f])|([0-9a-f]):((:([0-9a-f])){1,6})|(([0-9a-f]):){1,2}(:([0-9a-f])){1,5}|(([0-9a-f]):){1,3}(:([0-9a-f])){1,4}|(([0-9a-f]):){1,4}(:([0-9a-f])){1,3}|(([0-9a-f]):){1,5}(:([0-9a-f])){1,2}|(([0-9a-f]):){1,6}:([0-9a-f])|:((:([0-9a-f])){1,7}|:)|(([0-9a-f]):){1,7}:)(%[\da-z]+)?(/\d*)?(?!:)\b
  color: f#ff00ff

- description: MAC addresses
  regex: (?i)\b((?<!:)([0-9a-f]{1,2}:){5}[0-9a-f]{1,2}(?!:)|(?<!\.)([0-9a-f]{4}\.){2}[0-9a-f]{4}(?!\.))\b
  color: f#5f61ad

- description: Date in (YYYY-MM-DD|MMM (YYYY|DD)|DD MMM (YYYY)?)[\WT_]hh:mm:ss.SSSSSS format (date, sec, msec, and nsec optional)
  regex: (?i)\b((\d{2,4}\-((0)?[1-9]|1[0-2])\-(3[0-1]|([1-2]\d)|(0)?[1-9])|(?P<MMM>jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\s(\d{4}|\s\d|(3[0-1]|([0-2]\d)))|(\d|(3[0-1]|([0-2]\d)))\s(?P=MMM)(\s\d{4})?)[\WT_])?((2[0-3])|[0-1]\d):[0-5]\d(:[0-5]\d)?((\.|,)\d{3,6})?(?!:)\b
  color: b#af5f00
