...
int mathexp_tokenize(struct session *ses, char *str) {
  ...
  while (*pti) {
    ...
    case EXP_VARIABLE:
      ...
      case '(':
        if (pta != buf3) {
          //							show_message(ses,
          //-1,
          //"#MATH EXP: PARANTESES FOUND INSIDE A NUMBER");
          return FALSE;
        }
        *pta++ = *pti++;
      ...
      case '.':
        *pta++ = *pti++;
        if (point >= 0) {
          //							show_message(ses,
          //-1,
          //"#MATH EXP: MORE THAN ONE POINT FOUND INSIDE A NUMBER");
          precision = 0;
          return FALSE;
        }
        point++;
    ...
    case EXP_OPERATOR:
      ...
      default:
        //						show_message(ses, -1,
        //"#MATH EXP: UNKNOWN OPERATOR: %c", *pti);
        return FALSE;
      }
      break;
    }
  ...
  if (level != 0) {
    //		show_message(ses, -1, "#MATH EXP: UNMATCHED PARENTHESES, LEVEL:
    //%d", level);
    return FALSE;
  }
...
void mathexp_compute(struct session *ses, struct link_data *node) {
  ...
  case 'd':
    if (tintoi(node->next->str3) <= 0) {
      //				show_message(ses, -1, "#MATHEXP: INVALID
      // DICE: %s", node->next->str3);
      value = 0;
    } else {
  ...
  case '!':
    ...
    break;
  default:
    //			show_message(ses, -1, "#COMPUTE EXP: UNKNOWN OPERATOR:
    //%c%c", node->str3[0], node->str3[1]);
    value = 0;
    break;
  }
